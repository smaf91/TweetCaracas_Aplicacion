
var json_usuario = {};
  
$(document).ready(function(){
  $("#URL").click(function(){
  var data_file = "http://0.0.0.0:8000/lista_usuarios/";
  var http_request = new XMLHttpRequest();
        
  http_request.onreadystatechange = function(){
    if (http_request.readyState == 4) {
      var result = JSON.parse(http_request.responseText);
      json_usuario = result;
      var items = [];
      items.push("<a href='"+result['url']+"'>link</a>");
      document.getElementById("URL").remove();
      $(items).appendTo("#link");
      
    }
  }

  http_request.open("POST", data_file, false);
  http_request.send();
  });
});

$(document).ready(function(){
  $("#subscribir").click(function(){
    
  var objeto_serializado = $("#insertar").serializeArray();
    
  var data_file = "http://0.0.0.0:8000/lista_usuarios/ot="+json_usuario['oauth_token']+"/os="+json_usuario['oauth_token_secret']+"/";
  var http_request = new XMLHttpRequest();
  json_usuario['pin'] = objeto_serializado[3]['value'];
  data_file = data_file + "p="+json_usuario['pin']+"/";
  
  http_request.open("POST", data_file, false);
  http_request.send();
  });
});

  ...
  http_request.onreadystatechange = function(){
    if (http_request.readyState == 4) {
      ...
      for (var i=0; i < result.length; i ++) {
        var items = [];
        items.push("<div id='mask' class='hero-unit' style='padding-left: 30px;'><table style='width: 100%'><td style='width:40px;' valign='top'> <button onClick='borrar_usuario(\""+result[i]['entrada']+"\")' id='borrar' class='btn btn-danger' style='margin: 10px; padding: 5px; padding-bottom: 0px; padding-top: 0px'> x </button></td><td style='font-size: small; width: 75%'><table><tr><p>"+result[i]['entrada']+"</p></tr></table></td><td></td></table></div>");

...
$(document).ready(function(){
    localStorage['tipo'] = "hashtag";
    var data_file = "http://0.0.0.0:8000/"+localStorage['usuario_autenticado']+"/mascaras/t=hashtag/";
    ...
    http_request.onreadystatechange  = function(){
      if (http_request.readyState == 4  ) {
          ...
          for (var i = 0, len=result.length; i<len; i++) {
            var items = []
            items.push("<div id='"+result[i]['fields']['hashtag']+"'>");
            items.push("<div id='mask' class='hero-unit' style='background: "+result[i]['fields']['color']+"'>");
            items.push("<table>");
            items.push("<td style='width:40px;' valign='top'> ");
            items.push("<center>");
            items.push("<button onClick='borrar_mascara(\""+result[i]['fields']['hashtag']+"\")' class='btn btn-danger' style='margin: 10px; padding: 5px; padding-bottom: 0px; padding-top: 0px'> x </button>");
            items.push("<center>");
            items.push("</td>");
            items.push("<td>");
